...
PRESERVE_CUSTOM_DOMAIN=${INFRA_PIPELINE_PRESERVE_CUSTOM_DOMAIN:-true}
...
CUSTOM_DOMAIN_FLAG=${INFRA_ENABLE_CUSTOM_DOMAIN:-true}
if [ "$PRESERVE_CUSTOM_DOMAIN" != "true" ]; then
  CUSTOM_DOMAIN_FLAG=false
fi

APPROVE=true \
STACK="production" \
INFRA_CREATE_PIPELINES=true \
INFRA_ENABLE_CUSTOM_DOMAIN="$CUSTOM_DOMAIN_FLAG" \
